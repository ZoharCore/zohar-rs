...
pub async fn load_mobs(conn: &SqlitePool) -> Result<Vec<ContentMob>, ContentError> {
    let rows = sqlx::query(
        "SELECT mob_id, code, name, mob_type, rank, battle_type, level, ai_flags, move_speed,
                attack_speed, aggressive_sight, attack_range
         FROM mob_proto
         ORDER BY mob_id",
    )
    .fetch_all(conn)
...
mod tests {
    ...
    async fn load_mobs_parses_ai_flags_and_empty_values() {
        ...
        sqlx::query(
            "INSERT INTO mob_proto (mob_id, code, name, mob_type, rank, battle_type, level, ai_flags, move_speed, attack_speed, aggressive_sight, attack_range)
             VALUES (101, 'MOB_101', 'wild dog', 'MONSTER', 'PAWN', 'MELEE', 1, 'NOMOVE|AGGR', 100, 100, 800, 175),
                    (102, 'MOB_102', 'stable boy', 'NPC', 'KING', 'RANGE', 70, NULL, 100, 100, 0, 300)",
        )
        .execute(&pool)
        .await
        .expect("seed mobs");

...
mod tests {
        ...
        T: BinRead<Args<'static> = ()>,
    {
        ...
        T::read_options(&mut cursor, Endian::Little, ()).expect("decode")
    }

    #[test]
    fn zero_opt_preserves_u8_wire_size() {
        let raw_none = encode(&ZeroOpt::<Empire>::none());
        let raw_some = encode(&ZeroOpt::some(Empire::Red));

        assert_eq!(raw_none, vec![0x00]);
        assert_eq!(raw_some, vec![0x01]);
        assert_eq!(decode::<ZeroOpt<Empire>>(&raw_none).0, None);
        assert_eq!(decode::<ZeroOpt<Empire>>(&raw_some).0, Some(Empire::Red));
    }

    #[test]
    fn zero_opt_preserves_u32_wire_size() {
        let raw_none = encode(&ZeroOpt::<NetId>::none());
        let raw_some = encode(&ZeroOpt::some(NetId(0x1234_5678)));

        assert_eq!(raw_none, vec![0x00, 0x00, 0x00, 0x00]);
        assert_eq!(raw_some, vec![0x78, 0x56, 0x34, 0x12]);
        assert_eq!(decode::<ZeroOpt<NetId>>(&raw_none).0, None);
        assert_eq!(
            decode::<ZeroOpt<NetId>>(&raw_some).0,
            Some(NetId(0x1234_5678))
        );
    }
}

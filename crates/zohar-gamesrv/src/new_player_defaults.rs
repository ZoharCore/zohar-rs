...
impl EmpireStartMaps {
    pub fn from_options(red: Option<String>, yellow: Option<String>, blue: Option<String>) -> Self {
        ...
        Self {
            red: red.map(MapId::new).unwrap_or(defaults.red),
            yellow: yellow.map(MapId::new).unwrap_or(defaults.yellow),
            blue: blue.map(MapId::new).unwrap_or(defaults.blue),
        }
    }

    pub fn validate(self) -> Result<Self> {
        if self.red.as_str().trim().is_empty() {
            bail!("red start map code cannot be empty");
        }
        if self.yellow.as_str().trim().is_empty() {
            bail!("yellow start map code cannot be empty");
        }
        if self.blue.as_str().trim().is_empty() {
            bail!("blue start map code cannot be empty");
        }
        Ok(self)
    }

    pub fn map_id_for_empire(&self, empire: Empire) -> &MapId {
        match empire {
            Empire::Red => &self.red,
